...
**STATUS**: 1/10 Complete, 4 Active (P0/P1), 5 Deferred (P2/P3)
...
| 2 | MAINT-2 | Split handler.rs by stage | Backlog | Code | Implicit (maintainability) | | | | handler.rs (2,038 LOC) | **Problem**: handler.rs god object (2k LOC) mixes 6-stage orchestration. SPEC defines distinct stages. **Fix**: Create spec_kit/stages/{plan,tasks,implement,validate,audit,unlock}.rs (6 modules ~300-400 LOC each). Keep orchestration in handler (<1k LOC). **Acceptance**: handler.rs <1,000 LOC, 6 modules created, 178 tests pass. **Effort**: 1-2 days (M). |
| 3 | MAINT-3 | Implement test coverage Phase 1 | Backlog | Code | DOC-5 policy | | | | docs/spec-kit/testing-policy.md | **Problem**: 1.7% coverage (178/7,883 LOC), policy targets 40% by Q1 2026. Phase 1 (Nov 2025): Infrastructure. **Fix**: Implement MockMcpManager, create fixture library (20 agent outputs), setup cargo tarpaulin. **Acceptance**: MockMcpManager works, 20 fixtures extracted, tarpaulin reports 1.7% baseline. **Effort**: 2 weeks (L). |
| 4 | MAINT-4 | Evidence archival automation | Backlog | Code | DOC-4 policy | | | | docs/spec-kit/evidence-policy.md | **Problem**: 38 MB evidence (approaching 500 MB limit), manual cleanup only. Policy defines retention (unlock+30d), archival (compress/offload). **Fix**: Create evidence_archive.sh (compress consensus >30d), evidence_cleanup.sh (offload >90d). Update /spec-evidence-stats to warn >25 MB. **Acceptance**: Archives compress ≥70%, dry-run mode, warnings work. **Effort**: 1-2 days (M). |
| 5 | MAINT-5 | Audit FORK-SPECIFIC markers | Backlog | Code | UPSTREAM-SYNC.md | | | | spec_kit/* (7,883 LOC) | **Problem**: Partial FORK-SPECIFIC markers. UPSTREAM-SYNC.md defines conflict strategy but markers incomplete. **Fix**: Add `// FORK-SPECIFIC (just-every/code): <reason>` to all fork code (spec_kit/*, app.rs MCP spawn, chatwidget SpecKitContext, core agent timeout). **Acceptance**: ≥20 markers, consistent format. **Effort**: 0.5 days (S). |
