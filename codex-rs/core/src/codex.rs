...
impl Session {
    ...
    pub(crate) async fn send_event(&self, event: Event) {
        ...
        let recorded_event = crate::protocol::RecordedEvent {
            id: event.id.clone(),
            event_seq: event.event_seq,
            order: event.order.clone(),
            msg: event.msg.clone(),
        };
        let rollout_items = vec![RolloutItem::Event(recorded_event)];
        self.persist_rollout_items(&rollout_items).await;
        ...
        let user_msgs: Vec<RolloutItem> = msgs
            ...
            .filter_map(|m| match m {
                EventMsg::UserMessage(ev) => {
                    Some(RolloutItem::Event(crate::protocol::RecordedEvent {
                        id: String::new(),
                        event_seq: 0,
                        order: None,
                        msg: EventMsg::UserMessage(ev),
                    }))
                }
                _ => None,
            })
