...
async fn test_tail_includes_last_response_items() -> Result<()> {
    ...
    let user_event_line = RolloutLine {
        timestamp: ts.to_string(),
        item: RolloutItem::Event(RecordedEvent { id: "test-id".into(), event_seq: 0, order: None, msg: EventMsg::UserMessage(UserMessageEvent {
            message: "hello".into(),
...
async fn test_tail_skips_trailing_non_responses() -> Result<()> {
    ...
    let shutdown_event = RolloutLine {
        ...
        item: RolloutItem::Event(RecordedEvent { id: "test-id".into(), event_seq: 0, order: None, msg: EventMsg::ShutdownComplete }),
    };
