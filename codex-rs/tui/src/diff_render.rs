...
mod tests {
    ...
    fn ui_snapshot_apply_multiple_files_block() {
        ...
        let original_a = "one\n";
        let modified_a = "one changed\n";
        let patch_a = diffy::create_patch(original_a, modified_a).to_string();
        changes.insert(
            PathBuf::from("a.txt"),
            FileChange::Update {
                unified_diff: patch_a,
                move_path: None,
                original_content: original_a.to_string(),
                new_content: modified_a.to_string(),
            },
        );
