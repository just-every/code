...
fn status_snapshot_includes_reasoning_details() {
    ...
    let mut rendered_lines = render_lines(&composite.display_lines());
    if cfg!(windows) {
...
fn status_card_token_usage_excludes_cached_tokens() {
    ...
    let rendered = render_lines(&composite.display_lines());
...
fn status_context_window_uses_last_usage() {
    ...
    let rendered_lines = render_lines(&composite.display_lines());
