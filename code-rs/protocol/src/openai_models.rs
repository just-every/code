...
fn canonical_reasoning_efforts() -> impl Iterator<Item = ReasoningEffort> {
    [
...
mod tests {
    ...
    #[test]
    fn reasoning_effort_from_str_accepts_custom_values() {
        assert_eq!(
            "unsupported".parse::<ReasoningEffort>(),
            Ok(ReasoningEffort::Custom("unsupported".to_string()))
        );
        assert_eq!(
            serde_json::to_string(&ReasoningEffort::Custom("custom-fast".to_string())).unwrap(),
            "\"custom-fast\""
        );
    }

    #[test]
    fn reasoning_effort_from_str_rejects_empty_values() {
        assert_eq!(
